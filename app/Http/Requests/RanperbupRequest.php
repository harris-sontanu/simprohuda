...
class RanperbupRequest extends FormRequest
{
    /**
    ...
    public function rules(Request $request)
    {
        ...
        $requirements = Requirement::mandatory(2)->get();
        ...
        switch ($this->method()) {
            ...
            case 'PATCH':
                $rules['slug'] = [
                    'required',
                    Rule::unique('legislations')->ignore($this->route('ranperbup'))
                ];
                ...
                if ($request->has('post') OR $request->has('revise')) {
                    ...
                    foreach ($requirements as $requirement) {  
                        $document = Document::where('legislation_id', $this->route('ranperbup')->id)
