            ...
            <div class="card-header card-header d-sm-flex py-sm-0">
                <div class="py-sm-3 mb-sm-0 mb-3">
                    <div class="form-control-feedback form-control-feedback-end">
                        <form action="{{ route('legislation.ransk.index') }}" method="get">
                        ...
                        <ul class="nav navbar-nav flex-row flex-fill">
                            @foreach ($tabFilters as $key => $value)
                                ...
                                <li class="nav-item me-1">
                                    <a href="{{ route('legislation.ransk.index', ['tab' => $key] + Request::all()) }}" class="navbar-nav-link rounded{{ $active }}">
                            ...
                            <ul class="navbar-nav ms-lg-auto mt-2 mt-lg-0">
                                <li class="nav-item dropdown ms-lg-1">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('legislation.ransk.index', ['limit' => 25] + Request::all()) }}" class="dropdown-item" data-rows="25">25</a>
                                        <a href="{{ route('legislation.ransk.index', ['limit' => 50] + Request::all()) }}" class="dropdown-item" data-rows="50">50</a>
                                        <a href="{{ route('legislation.ransk.index', ['limit' => 100] + Request::all()) }}" class="dropdown-item" data-rows="100">100</a>
                                        <a href="{{ route('legislation.ransk.index', ['limit' => 200] + Request::all()) }}" class="dropdown-item" data-rows="200">200</a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'reg_number') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('legislation.ransk.index', ['order' => 'reg_number', 'sort' => $sortState] + Request::all()) }}" class="text-body d-block"><abbr title="Nomor Urut Registrasi" data-bs-popup="tooltip">Nomor</abbr></a>
                            </th>
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'title') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('legislation.ransk.index', ['order' => 'title', 'sort' => $sortState] + Request::all()) }}" class="text-body d-block">Judul</a>
                            </th>
                            @cannot('isOpd')                                
                                <th class="sorting @if (!empty($sort) AND Request::get('order') == 'institute') {{ 'sorting_' . $sort }} @endif">
                                    <a href="{{ route('legislation.ransk.index', ['order' => 'institute', 'sort' => $sortState] + Request::all()) }}" class="text-body d-block">Perangkat Daerah</a>
                                </th>
                            @endcannot
                            <th>Status</th>
                            ...
                            <th class="text-nowrap sorting @if (!empty($sort) AND Request::get('order') == 'created_at') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('legislation.ransk.index', ['order' => 'created_at', 'sort' => $sortState] + Request::all()) }}" class="text-body d-block">Tgl. Dibuat</a>
                            </th>
                            @endif
                            @if (Request::get('tab') !== 'draf')                                
                                <th class="text-nowrap sorting @if (!empty($sort) AND Request::get('order') == 'posted_at') {{ 'sorting_' . $sort }} @endif">
                                    <a href="{{ route('legislation.ransk.index', ['order' => 'posted_at', 'sort' => $sortState] + Request::all()) }}" class="text-body d-block">Tgl. Diajukan</a>
                                </th>
                            @endif
                            @if (in_array(Request::get('tab'), ['revisi', 'valid']))                                
                                <th class="text-nowrap sorting @if (!empty($sort) AND Request::get('order') == 'revised_at') {{ 'sorting_' . $sort }} @endif">
                                    <a href="{{ route('legislation.ransk.index', ['order' => 'revised_at', 'sort' => $sortState] + Request::all()) }}" class="text-body d-block">Tgl. Revisi</a>
                                </th>
                            @endif
                            @if (Request::get('tab') === 'valid')                                
                                <th class="text-nowrap sorting @if (!empty($sort) AND Request::get('order') == 'posted_at') {{ 'sorting_' . $sort }} @endif">
                                    <a href="{{ route('legislation.ransk.index', ['order' => 'posted_at', 'sort' => $sortState] + Request::all()) }}" class="text-body d-block">Tgl. Valid</a>
                                </th>
                            @endif
                            ...
                            <tr>
                                ...
                                <td class="text-center safezone">
                                    <div class="d-inline-flex">
                                        <a href="{{ route('legislation.ransk.show', $legislation->id) }}" class="text-body" data-bs-popup="tooltip" title="Pratinjau"><i class="ph-eye"></i></a>
                                        @if ($onlyTrashed)
                                            <form action="{{ route('legislation.ransk.restore', $legislation->id) }}" method="POST">
                                                @method('PUT')
                                                @csrf
                                            ...
                                            <form class="delete-form" action="{{ route('legislation.ransk.force-destroy', $legislation->id) }}" method="POST">
                                                @method('DELETE')
                                                @csrf
                                        ...
                                        @else
                                            @if ($legislation->status() !== 'validated')  
                                                <a href="{{ route('legislation.ransk.edit', $legislation->id) }}" class="text-body mx-2" data-bs-popup="tooltip" title="Ubah"><i class="ph-pen"></i></a>
                                                <form action="{{ route('legislation.ransk.destroy', $legislation->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
